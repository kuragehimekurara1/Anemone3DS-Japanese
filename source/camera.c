...
static void update_ui(void *arg)
{
    ...
    while(svcWaitSynchronization(data->event_stop, 2 * 1000 * 1000ULL) == 0x09401BFE) // timeout of 2ms occured, still have 14 for copy and render
    {
        ...
        set_screen(bottom);
        draw_text_center(GFX_BOTTOM, 4, 0.5, 0.5, 0.5, colors[COLOR_WHITE], "\uE005を押して終了します");
        end_frame();
    }
    ...
    if(finished && ready)
    {
        ...
        if(zip_size != 0)
        {
            ...
            if(r == ARCHIVE_OK)
            {
                ...
                {
                    throw_error("ダウンロードされたzipは\nスプラッシュでもテーマでもありません。", ERROR_LEVEL_WARNING);
                }
            }
            else
            {
                throw_error("ダウンロードしたファイルはzipファイルではありません。", ERROR_LEVEL_WARNING);
            }
            free(zip_buf);
        }
        else
        {
            throw_error("ダウンロードに失敗しました。", ERROR_LEVEL_WARNING);
        }
